...
int nAnonymizeINCOGNITOAmount = 1000;
...
#else
    const char* pszModule = "incognito";
#endif
    if (pex)
        return strprintf(
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "incognito.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty incognito.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override incognito.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "incognitod.pid"));
