...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::INCOGNITO)
    {
    }
...
void Coinmix::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("INCOGNITO")
    //updateDisplayUnit();
}
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizeINCOGNITOAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeINCOGNITOAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeINCOGNITOAmount = strAnonymizeINCOGNITOAmount.remove(strAnonymizeINCOGNITOAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeINCOGNITOAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeINCOGNITOAmount * COIN) nMaxToAnonymize = nAnonymizeINCOGNITOAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeINCOGNITOAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeINCOGNITOAmount));
        strAnonymizeINCOGNITOAmount = strAnonymizeINCOGNITOAmount.remove(strAnonymizeINCOGNITOAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeINCOGNITOAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void Coinmix::toggleDarksend()
{
    ...
    if (!fEnableDarksend) {
        ...
        if (nAnonymizeINCOGNITOAmount == 0) {
