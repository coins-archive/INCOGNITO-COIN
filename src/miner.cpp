...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("INCOGNITOMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("INCOGNITOMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("INCOGNITOMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("incognito-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("INCOGNITOMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running INCOGNITOMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadINCOGNITOMiner() exception");
    } catch (...) {
        LogPrintf("ThreadINCOGNITOMiner() exception");
    }

    LogPrintf("ThreadINCOGNITOMiner exiting\n");
}
